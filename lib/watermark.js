...
class Watermark {
    ...
    load(props, modified = true) {
        ...
        this.onWatermarkNullProp =
            props.onWatermarkNull || this.onWatermarkNullProp;
        this.imageWidth =
            props.imageWidth || this.imageWidth || defaultValues.imageWidth; // 水印图片宽度
        this.imageHeight =
            props.imageHeight || this.imageHeight || defaultValues.imageHeight; // 水印图片高度
        this.fontWeight =
            props.fontWeight || this.fontWeight || defaultValues.fontWeight; // 字体的粗细
        this.fontSize = props.fontSize
            ? `${fontSize}px`
            : this.fontSize || `${defaultValues.fontSize}px`; // font-size px
        ...
        this.globalAlpha =
            props.alpha || this.globalAlpha || defaultValues.alpha; // 水印文本透明度 0~1 0 表示完全透明，1 表示完全不透明
        ...
        if (!modified) {
            this.parentElement =
                props.parentElement ||
                document.getElementById(props.parentElementId) ||
                document.body;
        }
        // 水印动态宽高
        ...
        this.backgroundPosition =
            ...
            this.backgroundPosition ||
            defaultValues.backgroundPositionƒ; // 水印背景图片位置 background-position
    ...
    getDynamicWidthHeight = ({
        ...
        if (content) {
            ...
            if (this.parentElement) {
                ...
                console.log(
                    `this.parentElement is not existed`,
                    this.parentElement
                );
            }
        } else {
        ...
        let contentWidth =
            Math.ceil(
                Math.abs(Math.cos(radian) * watermarkContentOffsetWidth)
            ) + fontSize; // 单个水印宽度
        // eslint-disable-next-line max-len
        const contentHeight =
            Math.ceil(
                Math.abs(Math.sin(radian) * watermarkContentOffsetWidth)
            ) + fontSize; // 单个水印高度
        ...
        if (tip) {
            ...
            tipWidth =
                Math.ceil(
                    Math.abs(Math.cos(radian) * watermarkTipOffsetWidth)
                ) + fontSize; // 单个水印content宽度
            // eslint-disable-next-line max-len
            tipHeight =
                Math.ceil(
                    Math.abs(Math.sin(radian) * watermarkTipOffsetWidth)
                ) + fontSize; // 单个水印content高度
            if (tipWidth < width) {
                tipWidth = width;
        ...
        return new Promise((resolve) => {
            ...
            if (this.content) {
                ...
                if (this.tip) {
                    ...
                    // 平移转换，修改原点
                    context.translate(
                        this.dynamicWidthHeight.contentWidth / 2,
                        this.height / 2
                    );
                    // 旋转转换(弧度数)
                    context.rotate(this.rotate);
                    ...
                    // 平移转换，修改原点
                    context.translate(
                        this.dynamicWidthHeight.tipWidth / 2 +
                            this.dynamicWidthHeight.contentWidth,
                        this.height / 2
                    );
                    // 旋转转换(弧度数)
                    context.rotate(this.rotate);
    ...
    create = async () => {
        ...
        try {
            ...
            this.watermark.id = Date.now();
            this.watermarkInner.id = Date.now() + 1234;
            // 占用影响水印显示的css 属性
