...
class Watermark {
    ...
    load(props, modified = true) {
        ...
        if (!props || !this.validContentOrImage(props.content, props.image, props.onWatermarkNull)) {
            return;
        }
        ...
        this.rotate = props.rotate ? (props.rotate * Math.PI) / 180 : this.rotate || (defaultValues.rotate * Math.PI) / 180; // 水印旋转弧度，以左上角为原点旋转，注意旋转角度影响水印文本显示
        if (!modified) {
            this.parentElement = props.parentElement || document.getElementById(props.parentElementId) || document.body;
        }
        // 水印动态宽高
        ...
        this.backgroundPosition = props.backgroundPosition || this.backgroundPosition || defaultValues.backgroundPositionƒ; // 水印背景图片位置 background-position
