...
class Watermark {
    ...
    load(props, modified = true) {
        this.watermarkId = props.watermarkId || this.watermarkId || watermarkId;
        if (modified || document.getElementById(this.watermarkId)) this.destroy();
        if (
            !props ||
        ...
        this.parentObserve = null; // body监听
    ...
    create = async () => {
        ...
        try {
            ...
            this.watermark.id = this.watermarkId;
            // 占用影响水印显示的css 属性
            ...
            this.parentObserveWatermarkDom();
    ...
    destroy = () => {
        ...
        var watermarkElement = document.getElementById(this.watermarkId);
        removeDOM(watermarkElement);
        this.image = "";
        this.onDestroy?.();
    };

    // 监听水印节点dom变化，重新渲染
    parentObserveWatermarkDom = () => {
        // 选择将观察突变的节点
