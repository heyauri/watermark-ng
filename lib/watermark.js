...
class Watermark {
    ...
    load(props, modified = true) {
        this.outerId = props.outerId || this.outerId || outerId;
        this.innerId = props.innerId || this.innerId || innerId;
        if (modified || document.getElementById(this.outerId) || document.getElementById(this.innerId)) this.destroy();
        if (
            !props ||
        ...
        this.onDestroy = props.onDestroy || this.onDestroy;
    ...
    create = async () => {
        ...
        try {
            ...
            this.watermark.id = this.outerId;
            this.watermarkInner.id = this.innerId;
            // 占用影响水印显示的css 属性
    ...
    destroy = () => {
        this.watermarkObserve?.disconnect?.();
        this.bodyObserve?.disconnect?.();
        removeDOM(this.watermark);
        var outerElement = document.getElementById(this.outerId);
        removeDOM(outerElement);
        var innerElement = document.getElementById(this.innerId);
        removeDOM(innerElement);
        this.image = "";
        this.onDestroy?.();
    };
                ...
                if (item.removedNodes.length) {
                    for (var i = 0; i < item.removedNodes.length; i++) {
                        if (item.removedNodes[i] === this.watermarkInner) {
                            isChangeWaterMark = true;
                            if (this.onWatermarkRemove && typeof this.onWatermarkRemove == "function") {
